---
title: "EDY Analysis CCLine for SCC-lung"
...
LoY <- read_csv("LoY_cell_info_lung.cvs")
...
```{r}
pData <- AnnotatedDataFrame(LoY[-9,])
rownames(pData) <- LoY$CCLE_Name[-9]
```
...
```{r}
write_csv(EDYLoY, "Cell_EDYLoY_lung.cvs")
```
...
```{r}
 library(pheatmap)
  ...
  pheatmap(choose_matrix,show_colnames = F,annotation_col = annotation_col, show_rownames = T
           #,filename = 'CellLine_LOY_EDY_Signature.png'
           )
