---
title: "EDY Analysis CCLine for AdenoCar-lung"
...
We upload the curated metadata info extrated from the LoY pipeline for lung SCC:

```{r, message=FALSE, warning=FALSE}
LoY <- read_csv("LoY_cell_info_lung_adenocar.cvs")
...
```{r}
pData <- AnnotatedDataFrame(LoY[-28,])
rownames(pData) <- LoY$CCLE_Name[-28]
```
...
```{r}
write_csv(EDYLoY, "Cell_EDYLoY_lung_adenocar.cvs")
```
...
annoCol<-list(LoY= c(No= "palegreen2", Yes = "palevioletred1"), EDY = c(No = "skyblue1", Yes ="sandybrown"))
 library(pheatmap)
  ...
  pheatmap(choose_matrix,show_colnames = F,annotation_col = annotation_col, show_rownames = T, annotation_colors = annoCol
          ,filename = 'CellLine_LOY_EDY_Signature_lung_adenocar.png'
           )
